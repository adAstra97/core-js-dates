...
function getDayName(date) {
  ...
  const weekday = neededDate.getDay();
  let result;
  switch (weekday) {
    case 0:
      result = 'Sunday';
...
function getNextFriday(date) {
  ...
  const daysToFriday = currentDay >= 5 ? 7 - currentDay + 5 : 5 - currentDay;
...
function getCountDaysInMonth(month, year) {
  ...
  const currMonth = new Date(year, month);
  return (currMonth - prevMonth) / 1000 / 60 / 60 / 24;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (secondDate - firstDate) / 1000 / 60 / 60 / 24 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const finishPeriod = new Date(period.end);
  return currentDate >= startPeriod && currentDate <= finishPeriod;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 12; i += 1) {
    const day13 = new Date(year, i, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      resultArr.push(setDateStr(startDate));
